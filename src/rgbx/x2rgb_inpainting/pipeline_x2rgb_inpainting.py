...
class StableDiffusionAOVDropoutPipeline(
    ...
    def __call__(
        ...
        # Fred
        encoded_background=None,
        inpainting_mask=None,
        positive_guidance_image=None,
        negative_guidance_image=None,
        positive_mask=None,
        negative_mask=None,
        object_mask=None,
        relighting_guidance_scale=0.0,
        latent_mask_weight=[0.05, 0.05],
        latent_mask_boost=2.0,
        render_for_post_comp=False
    ):
        r"""
        ...
        do_relighting_guidance = (
            relighting_guidance_scale >= 1.0
        )

        assert not (do_relighting_guidance and do_classifier_free_guidance)
        ...
        if do_relighting_guidance:
            ...
            shadow_composites = torch.cat([positive_guidance_image, negative_guidance_image])
            shadowed_latents = self.vae.encode(shadow_composites.to(self.vae.dtype) * 2 - 1).latent_dist.sample() * self.vae.config.scaling_factor

            shadow_guidances = torch.cat([positive_mask, negative_mask])[:,0:1,:,:]
            latent_shadow_guidances = F.interpolate(shadow_guidances, size=(64, 64), mode='bilinear')
            object_mask_lr = F.interpolate(object_mask, size=(64, 64), mode='bilinear')

            epsilon_shadow_mask = 0.05
            
            binary_shadow_mask = (latent_shadow_guidances > epsilon_shadow_mask).float()
            binary_obj_mask = (object_mask_lr > epsilon_shadow_mask).float()
            ...
            final_latent_mask = eroded_latent_shadow_mask + edge_latent_mask * latent_mask_boost
        ...
        for aov_name in required_aovs:
            ...
            if aov_name == "normal":
                if normal is not None:
                    preprocessed_aovs[aov_name] = (
                        self.image_processor.preprocess_normal(normal.unsqueeze(0) * (1 - mask))[0]
                    )
                else:
                    preprocessed_aovs[aov_name] = None
            ...
            if aov_name == "irradiance":
                if irradiance is not None:
                    preprocessed_aovs[aov_name] = self.image_processor.preprocess(
                        irradiance.unsqueeze(0) * (1 - mask)
                    )
                else:
                    preprocessed_aovs[aov_name] = None
        for k, v in preprocessed_aovs.items():
            if v is not None:
                print('a', k, v.shape)
            else:
                print('a', k, 'None')
        ...
        for aov_name, aov in preprocessed_aovs.items():
            if aov is None:
                ...
                elif do_relighting_guidance:
                    image_latents.append(
                        torch.cat([image_latent, image_latent], dim=0)
                    )
                else:
                ...
                if aov_name == "irradiance":
                    ...
                    image_latent = (
                        self.prepare_image_latents(
                            ...
                            device,
                            do_classifier_free_guidance,
                            do_relighting_guidance=False,
                            generator=generator,
                        )
                        * scaling_factor
                    )
                    print('after', aov_name, image_latent.shape)
                image_latents.append(image_latent)
        ...
        mask_latents = F.interpolate(
            mask.to(device=device, dtype=prompt_embeds.dtype),
            size=(height_latent, width_latent),
            mode="bilinear",
            align_corners=False,
            antialias=True,
        ...
        if do_relighting_guidance:
            ...
            # image_latents.append(torch.cat([mask_latents, mask_latents], dim=0))
            image_latents.append(mask_latents)
        else:
            image_latents.append(masked_image_latents)
            image_latents.append(mask_latents)
        
        for e in image_latents:
            print(e.shape)
        ...
        noise = randn_tensor(latents.shape, generator=generator, device=latents.device, dtype=latents.dtype) * self.scheduler.init_noise_sigma
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                elif do_relighting_guidance and latents.shape[0] == 1: # TODO: clean up this...
                ...
                noise_pred = self.unet(
                    scaled_latent_model_input.to(torch.float16),
                    t,
                ...
                if do_relighting_guidance:
                    # TODO: is rescale_noise_cfg required?
                    (
                        pred_positive,
                        pred_negative,
                    ) = noise_pred.chunk(2)
                    noise_pred = (1 - object_mask_lr) * pred_positive + \
                                    object_mask_lr * (
                                        relighting_guidance_scale * (
                ...
                if i < len(timesteps) - 1:
                    t_noised_bg = timesteps[i+1]
                else:
                    t_noised_bg = torch.zeros_like(timesteps[i])
                
                # inpainting the bg
                # noise = randn_tensor(latents.shape, generator=generator, device=latents.device, dtype=latents.dtype)
                # if do_relighting_guidance:
                #     noise = torch.cat([noise[0:1], noise[0:1]]) # TODO: clean
                # noised_bg = self.scheduler.add_noise(encoded_background, noise, t_noised_bg)
                # if do_relighting_guidance or render_for_post_comp:
                #     inpainting_mask_positive = 1 - (1 - inpainting_mask) * (1 - latent_mask[0:1])
                #     latents = latents[...] * inpainting_mask_positive + noised_bg * (1 - inpainting_mask_positive)
                # else:
                #     latents = latents[...] * inpainting_mask + noised_bg * (1 - inpainting_mask)
                # TODO: remove unneeded params

                # add shadows
                if do_relighting_guidance and i < len(timesteps) - 1 and not render_for_post_comp:
                    noised_hard_shadow_latent = self.scheduler.add_noise(shadowed_latents, noise, t) # TODO: shift t
                    weighted_latent_mask = torch.cat([final_latent_mask[0:1] * latent_mask_weight[0], final_latent_mask[1:2] * latent_mask_weight[1]])
                    latents = weighted_latent_mask * noised_hard_shadow_latent + (1 - weighted_latent_mask) * latents[...]
