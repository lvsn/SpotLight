...
# 
    ...
    for batch_idx, batch in enumerate(tqdm(val_dataloader)):
        ...
        while os.path.exists(os.path.join(args.shadows_dir, scene_name, f'shadow_positive_{shadow_map_idx:04d}.png')):
            all_frames.append(f'{shadow_map_idx:04d}')
            shadow_map_idx += 1

        if os.path.exists(os.path.join(args.shadows_dir, scene_name, f'shadow_positive_gt_dir.png')):
            all_frames.append('gt_dir')

        for frame_idx, frame_name in enumerate(all_frames):
            if args.eval.render_only_gt_dir and frame_name != 'gt_dir':
                continue

            shadow_custom = batch[f'shadow_positive_{frame_name}'] * (1 - batch['mask'])                    
            shadow_opposite = batch[f'shadow_negative_{frame_name}'] * (1 - batch['mask'])
            conditioning_negative, _, _, _, negative_image_logs, negative_shading_mask = compositor.composite_batch(
                batch=batch, 
                args=args, 
                to_predictors=to_predictors, 
